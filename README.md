...
3. 使用 **技能编辑器** (`Tech-Cosmos → SkillSystem → Skill Editor Window`) 配置：
   ...
   - 条件列表：冷却条件、生命值条件等
   - 机制列表：选择刚创建的伤害机制
   - 数据层：必要数据会自动出现（如 DamageMultiplier），也可手动添加公式值
...
```csharp
[RequiredData("DamageMultiplier", typeof(float), DefaultValue = "1.5", Description = "伤害倍率")]
[RequiredData("CriticalChance", typeof(float), DefaultValue = "0.1", Description = "暴击率")]
public class DamageMechanism<T> : Mechanism<T> where T : class, IUnit<T>
{
    // ...
}
...
| `[RequiredData]` | Class | 声明机制/条件所需的数据项 |
...
public class RequiredDataAttribute : Attribute
{
    ...
    public string Description { get; set; }   // 描述（显示在编辑器中）
}
```
...
### 9.5 使用 RequiredData 声明数据依赖

```csharp
[Serializable]
[RequiredData("HealAmount", typeof(float), DefaultValue = "50", Description = "治疗量")]
[RequiredData("HealRatio", typeof(float), DefaultValue = "0.3", Description = "治疗比率")]
[AutoGenerateMechanism(typeof(Character))]
...
public class HealMechanism<T> : Mechanism<T> where T : class, IUnit<T>
{
    public override void Execute(SkillContext<T> context, IDataLayer<T> dataLayer)
    {
        float baseHeal = dataLayer.GetValue<float>("HealAmount", context);
        float healRatio = dataLayer.GetValue<float>("HealRatio", context);
        float maxHealth = dataLayer.GetValue<float>("MaxHealth", context);

        float totalHeal = baseHeal + maxHealth * healRatio;

        // context.Target.Heal(totalHeal);
    }
}
```
...
2. **缓存频繁访问的数据**：使用 `CachedCondition<T>` 缓存昂贵计算
...
5. **使用 RequiredData 声明依赖**：让编辑器自动管理必要数据项
...
│   │
│   └── Attributes/                    # 标记特性
│       ├── RequiredDataAttribute.cs    # 必要数据声明
...
### Q4: RequiredData 声明后编辑器没有自动创建数据项？

**A**: 确保：
1. 运行了 `Generate ALL Classes` 重新生成
2. 在技能编辑器中打开了对应的 SkillDataSO
3. 机制/条件已添加到技能中（编辑器会根据实际使用的机制/条件来收集 RequiredData）
...
### Q7: 如果两个不同的机制/条件对同一个 key 声明了不同类型的 RequiredData 会怎样？

**A**: 编辑器会自动检测类型冲突，并在 Console 中输出详细的错误信息，包含冲突的来源和期望的类型。