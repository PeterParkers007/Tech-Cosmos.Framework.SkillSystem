...
namespace TechCosmos.SkillSystem.Runtime
{
    /// <summary>
    /// 标记机制/条件所需的数据项。
    /// 添加后会在编辑器中自动填充默认值，并同步到对应条目；删除机制/条件时自动移除。
    /// </summary>
    ...
    public class RequiredDataAttribute : Attribute
    {
        ...
        /// <summary>描述文字（显示在数值条目标题）。</summary>
        public string Description { get; set; }

        /// <summary>是否为公式类型；为 true 时自动创建 FormulaValue。</summary>
        public bool IsFormula { get; set; }

        /// <summary>公式类型（IsFormula=true 时有效）。</summary>
        public FormulaValue.FormulaType FormulaType { get; set; } = FormulaValue.FormulaType.Static;

        /// <summary>公式的静态默认值（IsFormula=true 时有效）。</summary>
        public float StaticValue { get; set; }

        /// <summary>公式的引用路径（IsFormula=true 且 FormulaType=Reference 时有效）。</summary>
        public string ReferencePath { get; set; }

        /// <summary>公式的自定义表达式（IsFormula=true 且 FormulaType=Custom 时有效）。</summary>
        public string CustomFormula { get; set; }

        /// <summary>
        /// 允许序列化的类型白名单。
        /// 为 null 或空数组表示允许所有类型。
        /// 例如 new[] { typeof(float), typeof(FormulaValue) } 只允许 Float 与 Formula 序列化。
        /// </summary>
    ...
    {
        /// <summary>在 Inspector 中的分类。</summary>
        public string Category { get; set; } = "技能数据";
        ...
        /// <summary>是否在条件编辑器中显示。</summary>
        public bool ShowInCondition { get; set; }

        /// <summary>是否在机制编辑器中显示。</summary>
        public bool ShowInMechanism { get; set; }
    }

    /// <summary>
    /// 标记 SkillDataSO 应包含的自定义数据条目。
    /// </summary>
    [AttributeUsage(AttributeTargets.Field | AttributeTargets.Property, Inherited = false)]
    ...
    /// <summary>
    /// 标记条件类需要为指定 Unit 类型自动生成泛型条件。
    /// </summary>
    [AttributeUsage(AttributeTargets.Class, Inherited = false)]
    public class AutoGenerateConditionAttribute : Attribute
    {
        /// <summary>目标 Unit 类型；未指定时为所有 IUnit 子类型生成。</summary>
    ...
    {
        /// <summary>目标 Unit 类型（可选；未指定时为所有 IUnit 子类型生成）。</summary>
        public Type[] TargetTypes { get; }
    ...
    public class MechanismMenuAttribute : Attribute
    {
        ...
        /// <summary>显示名称（可选，默认使用类名）。</summary>
        public string DisplayName { get; set; }
    ...
    /// </summary>
    [AttributeUsage(AttributeTargets.Class, Inherited = false)]
    public class ConditionMenuAttribute : Attribute
    {
        /// <summary>菜单分类。</summary>
        public string Category { get; }
        ...
        public ConditionMenuAttribute(string category)
        {
            Category = category;
        }
    }
